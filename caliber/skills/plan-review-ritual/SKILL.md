...
3. **plan 违反自己写的约束是最常见的坑。** 写完"铁律"后第一个违反它的
   往往是作者自己。
4. **plan 的读者是逐字执行的执行者，不是会心领神会的同事。** 执行模型
   越弱，review 的 ROI 越大。
...
- `CHECKLIST`：审查视角清单（可选）：plan-forge 工序 3 注入五视角；
  缺省用 Step 2 的通用模板。
- `VOICE_B`：第二声部 = **qwen-cli 外部声部（固定，非可让渡首选项）**；故障走
  Step 2 修复梯子，降级/单声部均为**用户裁定停止点**（2026-09-01 裁定）。
...
1. **固定声部：qwen-cli 外部声部**（Qwen，与主声部不同模型家族，对抗价值最高；
   ...
   **调用规程以 qwen-cli skill 为唯一准绳**——预检、调用命令、stdin 重定向
   ...
   返回审查文本 = 成功；任何失败 → 第 2 条修复梯子（**先修复，禁止降级**）。
2. **失败 = 先修复（修复梯子，每步留痕），阶梯耗尽前禁止降级**。按 qwen-cli
   失败模式表定性后依次走：
...
3. **降级 = 用户裁定停止点**：修复梯子 ①-④ 耗尽仍失败 → **停**，把验尸
   ...
   通道（minimax-cli skill，先 30s ping `--print "Reply: PONG"`）或第二个
   独立 subagent（prompt 换框架）。**禁止自动降级、禁止自动 `[single-voice]`**
   （旧版「全不可用标记 single-voice 继续不中断」已废除——静默降级让外部
   声部形同虚设）。批准后降级原因 + 用户批准记录写入审计表。

**轮次化变体**（`ROUND≥2` 时启用；轮 1 模板一字不改）：

- **声部 A 加三段**：
  1. `DELTA` 段——上轮修复清单（ID / 修复位置 / 修复摘要 / 涟漪），立场 =
     **修复验尸**：每个修复都是嫌疑人，查它引入的新不一致、漏改的调用点/
     契约矩阵行、与全局约束的矛盾；
  2. **恶意字面执行者** stance——以最大字面主义、最小善意执行 plan，
     找"歧义让执行者做出作者意料之外的事"的行；
  3. **防重复纪律**——不重报已修项，除非修复本身错误/不完整。
- **声部 B 切换为纯文本 delta 猎手**（其无仓库约束不变）：恶意字面执行者 +
  **回填漂移**专责（修复段与未修复段的自相矛盾、悬空引用、字面歧义多解读）。
  轮 ≥2 的 B prompt **必须缩域**（delta + 文本一致性），禁止全量任务单
  （2026-09-01 实证：全量单被 300s 封顶误杀健康任务）。
- 轮次编排、stance 轮换表、收敛判据、重锻出口是**调用方策略**
  （plan-forge 工序 3），本 skill 只提供轮次化机制。
...
2. **门前验证**（验收前逐项核对，缺一补一，最多 2 轮）：
   ...
   - [ ] 每个检查项/视角都有"查了什么"记录
   - [ ] 未决项 = 0，或每条带重访触发器
3. **字面执行验收（litmus）**：假想一个不知道本项目任何背景的弱模型
   执行者，只看 plan 逐字执行——
