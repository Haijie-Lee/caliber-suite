...
## §TDD（替代 coding-forge §inline TDD 骨架）

先写失败测试 → 跑它看红 → 最小实现 → 跑它看绿 → commit。
没有测试的改动不算改动，算许愿。

## §证据（替代 coding-forge §验证手段菜单）

任何"完成/修好/通过"的声称之前：跑验证命令，把输出贴出来。
输出与声称不符时，以输出为准。

## §根因（替代 coding-forge §过程骨架菜单 调试骨架）

复现 → 最小化 → 定位到具体文件/行/条件 → 单变量验证假设 → 修 → 回归测试锁定。
禁止"改改看行不行"的散弹式修复。

## §三问（替代 caliber:deep-probe）

动手前问用户三个问题：要做什么（可观测的完成态）？为什么（约束与动机）？
什么绝对不能碰（不可逆边界）？答完才进计划阶段。

## §简plan（替代 caliber:plan-drafting——MS 轻量档的兜底简版）
