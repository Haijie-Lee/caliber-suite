...
轮 1（全量基线）── 五视角 + 双声部，现行模板一字不改
    │
    ├─ fast path：零 P1/P2、零 User Challenge 且 Mechanical 全部无生产文本层涟漪
    │   → 免轮 2/3；零修复 → 直接进工序 4；有静默修 → 工序 3.5 → 工序 4
    │
    └─ 有 P1/P2 或生产文本层涟漪修复 → 轮 2（delta 验尸 + 恶意字面执行者 + 完整性批判）
        │
        ├─ 收敛 → 工序 3.5 准出闸口 → 工序 4
        └─ 仍有 P1/P2 → 轮 3（pre-mortem + delta 复验）
            │
            ├─ 收敛 → 工序 3.5 准出闸口 → 工序 4
            └─ 轮 3 仍有 P1/P2 → **停**：三级阶梯①全量基线重审 → ②回工序 2 重锻 → ③用户裁定降级/缩范围（见下「封顶与出口」）
```

**操作化判据**（不许凭感觉）：

- **收敛** = 一轮内零 P1/P2 + 零 User Challenge + Mechanical 全部无生产文本层涟漪。
- **收敛的局限**（2026-09-08）：收敛度量**审查过程收敛**（问题清单清零），
  不度量**工件收敛**（plan 与原始目标/仓库现实的一致性）——后者由
  工序 3.5 准出闸口守护。判据达标 ≠ plan 没漂；宣告收敛时必须在同一行
  写明闸口待办，不许只报"已收敛"。
- **涟漪分层（1.9.0）**：**生产文本层** = 修复触碰规格/指令/契约值等生产文本
