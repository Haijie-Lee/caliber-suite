...
彩排发现 → 回工序 2 补全（不是嘴上答"显然"）。全过 → plan 末尾补总结
行 + 审计文件指针（发现数 / 分类分布 / 收敛轮次 / 未决项——无则
NO UNRESOLVED），进入交接。
...
- 调用方关系：caliber M/L 级阶段 2 = 工序 1-2（M 级仅此，审查回 caliber 阶段 3
  的 Step 1 自审）；L 级阶段 3 = 工序 3-4（收敛循环 + 准出闸口 + 真实彩排）。
